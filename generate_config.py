#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate docs/.vuepress/config.ts from the current VuePress 1 config.js"""
...
for i, line in enumerate(original_lines):
    if i > 700 and (line.strip().startswith('// other') or
                    line.strip().startswith('// getStudyRoadJava') or
                    (line.strip().startswith('function ') and 'get' in line)):
        func_start = i
        break
...
sidebar_funcs = ''.join(original_lines[func_start:])

# The config.ts header with all the main config
config_header = r"""import { defineUserConfig } from 'vuepress'
...
output_path = 'docs/.vuepress/config.ts'
